...
######## stage 5b: configmaps creation
oc create configmap rl-lyrics-page --from-file=config.js && rm config.js

######## stage 5c: configmaps mounting
oc set volume dc/rl-lyrics-page --add --sub-path=config.js -m /opt/app-root/src/public/config.js -t configmap --configmap-name=rl-lyrics-page

####### stage 6a: configuring mongodb
...
######## stage 6b: configmap creation
oc create cm rl-lyrics-mongodb-import --from-file=./lyrics.json

######## stage 6c: configmap mounting
oc set volume dc/rl-lyrics-mongodb --add -m /tmp/data -t configmap --configmap-name=rl-lyrics-mongodb-import

######## stage 6d: posthook creation for mongodb
...
oc patch dc/rl-hits-service -p '{"spec": {"template": {"spec": { "containers": [{"name": "rl-hits-service","readinessProbe": {"httpGet": {"path": "/health","port": 8080,"scheme": "HTTP"},"initialDelaySeconds": 60,"timeoutSeconds": 3,"periodSeconds": 10},"livenessProbe": {"httpGet": {"path": "/health","port": 8080,"scheme": "HTTP"},"initialDelaySeconds": 60,"timeoutSeconds": 3,"periodSeconds": 10}}]}}}}'
oc patch dc/rl-lyrics-page -p '{"spec": {"template": {"spec": { "containers": [{"name": "rl-lyrics-page","readinessProbe": {"httpGet": {"path": "/","port": 8080,"scheme": "HTTP"},"initialDelaySeconds": 60,"timeoutSeconds": 3,"periodSeconds": 10},"livenessProbe": {"httpGet": {"path": "/","port": 8080,"scheme": "HTTP"},"initialDelaySeconds": 60,"timeoutSeconds": 3,"periodSeconds": 10}}]}}}}'
oc patch dc/rl-songs-service -p '{"spec": {"template": {"spec": { "containers": [{"name": "rl-songs-service","readinessProbe": {"httpGet": {"path": "/actuator/health","port": 8080,"scheme": "HTTP"},"initialDelaySeconds": 60,"timeoutSeconds": 3,"periodSeconds": 10},"livenessProbe": {"httpGet": {"path": "/actuator/health","port": 8080,"scheme": "HTTP"},"initialDelaySeconds": 60,"timeoutSeconds": 3,"periodSeconds": 10}}]}}}}'
...
#### if mongodb's posthook fails then
